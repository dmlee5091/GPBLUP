Module M_Stamp
  ...
  ! Public interfaces
  public :: version, timestamp, timelock
  
  ! Private parameters
  integer, parameter :: lock_y=2030
  integer, parameter :: lock_m=12
  integer, parameter :: lock_d=31
  integer, parameter :: r8=8
contains
  subroutine version(ver)
  character(len=*), intent(in):: ver
  print '(a)', ' Program version: '//trim(ver)
  print '(a)', ' Developed by Dr. DEUKMIN LEE (Hankyong National Univ.)'
  print '(a)', ' E-mail: dhlee@hknu.ac.kr'
  end subroutine
  
  subroutine timestamp (io_u)  ! print a timestamp on UNIT io_u
  ...
  if(leftday.LE.60) then
    ...
    print*
    print '(a)', ' Please update new version - contact with Dr. DEUKMIN LEE'
